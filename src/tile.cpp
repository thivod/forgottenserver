...
void Tile::onUpdateTileItem(Item* oldItem, const ItemType& oldType, Item* newItem, const ItemType& newType)
{
	if (newItem->hasProperty(MOVEABLE) || newItem->getContainer()) {
		Game::BrowseFieldMap::const_iterator it = g_game.browseFields.find(this);
		if (it != g_game.browseFields.end()) {
			int32_t index = it->second->getIndexOfThing(oldItem);
			if (index != -1) {
...
void Tile::moveCreature(Creature* creature, Cylinder* toCylinder, bool forceTeleport/* = false*/)
{
	...
	int32_t oldStackPos = getIndexOfThing(creature);
	...
	int32_t newStackPos = newTile->getIndexOfThing(creature);
...
ReturnValue Tile::queryAdd(int32_t index, const Thing* thing, uint32_t count,
...
ReturnValue Tile::queryMaxCount(int32_t index, const Thing* thing, uint32_t count, uint32_t& maxQueryCount,
...
Cylinder* Tile::queryDestination(int32_t& index, const Thing* thing, Item** destItem,
                                   uint32_t& flags)
{
	Tile* destTile = NULL;
	*destItem = NULL;
	...
	if (creature) {
		...
		if (item->isGroundTile()) {
			if (ground == NULL) {
				...
				int32_t oldGroundIndex = getIndexOfThing(ground);
			...
			if (item->isSplash()) {
				...
				if (items) {
					for (ItemVector::iterator it = items->getBeginTopItem(); it != items->getEndTopItem(); ++it) {
						if ((*it)->isSplash()) {
							int32_t oldSplashIndex = getIndexOfThing(*it);
							Item* oldSplash = *it;
					...
					for (ItemVector::iterator it = items->getBeginDownItem(); it != items->getEndDownItem(); ++it) {
						...
						if (oldField) {
							if (oldField->isReplaceable()) {
								int32_t oldFieldIndex = getIndexOfThing(*it);
...
void Tile::removeThing(Thing* thing, uint32_t count)
{
	...
	if (item) {
		int32_t index = getIndexOfThing(item);
...
int32_t Tile::getIndexOfThing(const Thing* thing) const
{
	int32_t n = -1;
...
int32_t Tile::getFirstIndex() const
{
	return 0;
}

int32_t Tile::getLastIndex() const
{
...
Thing* Tile::getThing(uint32_t index) const
{
	if (ground) {
		if (index == 0) {
			return ground;
